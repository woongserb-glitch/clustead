...
도메인이 있으면 Let's Encrypt 적용:
1. `docker-compose.yml` 의 nginx `443` 포트 + certbot 볼륨 주석 해제.
2. `deploy/nginx/livefit.conf` 의 `server_name` 을 실제 도메인으로 교체, ACME challenge location 주석 해제.
3. certbot 으로 발급(예: `certbot/certbot` 이미지로 webroot 방식), `live/<도메인>` 인증서 생성.
4. `livefit.conf` 하단 443 server 블록 주석 해제 + 80→443 리다이렉트로 전환.
5. `docker compose up -d` 재적용. 갱신은 certbot renew + nginx reload.

---

## 부록: 남은 운영 과제 (A안 이후)
- 에러 트래킹(Sentry) · 메트릭 연동.
- 법무: 부동산 데이터 면책 고지 + 출처표기(서울 열린데이터/공공데이터/Kakao) + 기준일 표시.
