...
## 실거래 갱신 후 확인: 동(洞) 불일치 점검표

```bash
python scripts/build_transaction_summary.py
```

이 스크립트는 `transaction_summary.csv` 와 단지별 detail 샤드를 만들면서
**`data/reports/transaction_dong_mismatch.csv`** 를 함께 남긴다. 실행 로그 끝에
`[OK] 동 불일치 점검: 구제 N건 / 보류 M건` 이 찍힌다.

**왜 보나**: 실거래 조회 키에 동이 들어가는데, 단지 마스터의 동과 국토부
실거래의 동이 갈리는 단지가 있다. 그러면 실거래가 **통째로 안 붙고**, 화면에는
'거래 없음'과 똑같이 보여 조용히 묻힌다. 2026-08-06 에 7개 단지가 이 상태였다
(마곡 5곳 방화동↔마곡동, 광화문스페이스본 신문로2가↔사직동, 신도림현대
신도림동↔구로동).

| 상태 | 뜻 | 할 일 |
|---|---|---|
| `구제됨` | 동이 어긋났지만 같은 구에 같은 이름이 **한 동뿐**이라 자동 연결됨 | 건수만 확인. **갑자기 늘면** 상류 데이터의 동 표기가 흔들린 것 |
| `보류` | 같은 구에 같은 이름이 **두 동 이상** 있어 자동 판단을 포기함 | **사람이 확인.** 진짜 동명이 단지면 그대로 두고, 같은 단지가 쪼개진 것이면 `apartment_transaction_mapping_manual.csv` 로 수동 지정 |

`보류` 가 있어도 빌드는 깨지지 않는다 — 동명이 단지라면 그 상태가 정상이기
때문이다. 대신 로그에 `[확인필요]` 로 목록이 찍힌다.
